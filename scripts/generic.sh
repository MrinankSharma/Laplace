...
echo "srun --output ${HOME}/slurm_outputs/subset_laplace/${FILENAME}.out singularity exec --nv --bind ${DATA}:${DATA} ${DATA}/laplace_dev.sif python3 $JOB_CMD"
srun --output ${HOME}/slurm_outputs/subset_laplace/${FILENAME}.out singularity exec --nv --bind ${DATA}:${DATA} ${DATA}/laplace_dev.sif python3 $JOB_CMD